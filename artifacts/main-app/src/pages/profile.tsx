...
import { Mail, Wallet, CheckCircle, Clock, XCircle } from "lucide-react";
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <Card className="border-none shadow-sm bg-blue-50">
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1"><Wallet className="h-3 w-3" /> Balance</p>
              <p className="text-2xl font-bold text-blue-600">{profile?.walletBalance ?? 0} ETB</p>
            </CardContent>
          </Card>
          <Card className="border-none shadow-sm">
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1"><CheckCircle className="h-3 w-3 text-green-600" /> Approved</p>
              <p className="text-2xl font-bold">{profile?.approvedSubmissions ?? 0}</p>
            </CardContent>
          </Card>
          <Card className="border-none shadow-sm">
            <CardContent className="p-4">
              <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1"><Clock className="h-3 w-3 text-yellow-600" /> Pending</p>
        ...
        <Tabs defaultValue="submissions">
          ...
          <TabsContent value="submissions" className="mt-4">
            <Card className="border-none shadow-sm">
              <CardHeader><CardTitle className="text-sm font-semibold">All Submitted Accounts</CardTitle></CardHeader>
              <CardContent>
                {subsLoading ? (
                  ...
                  <div className="text-center py-10 text-muted-foreground">
                    ...
                    <Link href="/submit" className="text-blue-600 text-sm hover:underline mt-1 inline-block">Submit your first account</Link>
                  </div>
                ) : (
          ...
          <TabsContent value="withdrawals" className="mt-4">
            <Card className="border-none shadow-sm">
              <CardHeader><CardTitle className="text-sm font-semibold">Withdrawal History</CardTitle></CardHeader>
              <CardContent>
                {wdLoading ? (
                  <div className="space-y-3">{[...Array(3)].map((_, i) => <Skeleton key={i} className="h-12 w-full" />)}</div>
                  ...
                  <div className="text-center py-10 text-muted-foreground">
                    ...
                    <Link href="/withdraw" className="text-blue-600 text-sm hover:underline mt-1 inline-block">Request a withdrawal</Link>
                  </div>
                ) : (
                  <div className="divide-y">
                    {withdrawals.map((wd) => (
